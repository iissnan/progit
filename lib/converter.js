...
function Converter (translation) {
    ...
    this.isDefaultTranslation = translation === CONFIG.defaultTranslation;
    ...
    function isValidDefaultTranslation () {
        ...
        return (
            dt &&
            fs.existsSync(CONFIG.SOURCE_DIR + "/" + dt) &&
...
Converter.prototype.prepare = function () {
    ...
    !fs.existsSync(CONFIG.DESTINATION_DIR) && fs.mkdirSync(CONFIG.DESTINATION_DIR);
    !fs.existsSync(this.destination) && fs.mkdirSync(this.destination);

    this.indexPage = this.isDefaultTranslation ?
        "index.html" :
...
Converter.prototype.getTemplate = function () {
    ...
    var tplPage = "page.jade";
    var tplAbout = "about.jade";

    this.template = {
        indexCompiler: getCompiler(tplIndex),
        pageCompiler:  getCompiler(tplPage),
        aboutCompiler: getCompiler(tplAbout)
    };
    ...
    function getCompiler(tpl) {
        var compiler = jade.compile(
            fs.readFileSync(CONFIG.TEMPLATE_DIR + "/" + tpl, "utf-8"),
            {filename: CONFIG.TEMPLATE_DIR + "/" + tplLayout}
        );
...
Converter.prototype.getMenu = function (level) {
    ...
    for (var i = 0; i < CONFIG.supportTranslations.length; i++) {
        ...
        var chaptersPath = fs.readdirSync(CONFIG.SOURCE_DIR + "/" + translation);
        var chapters = chaptersPath.map(function(chapter) {
    ...
    function getCompiler (tpl) {
        return jade.compile(
            fs.readFileSync(CONFIG.TEMPLATE_DIR + "/" + tpl),
            "utf-8"
        );
    }
    ...
    function getTableOfContentsLink (translation) {
        ...
        var href = uriPrefix + (translation === CONFIG.defaultTranslation ?
                "index.html" :
...
Converter.prototype.getLabel = function (config) {
    ...
    var labelSectionDefault = I18N[config.section][CONFIG.defaultTranslation];
