...
var pathTemplate = __dirname + "/../template",
    ...
    templateIndex = jade.compile(fs.readFileSync(pathTemplateIndex, "utf-8"), {filename: pathTemplateLayout}),
    templatePages = jade.compile(fs.readFileSync(pathTemplatePages, "utf-8"), {filename: pathTemplateLayout});
            ...
            if (chapterContent) {
                ...
                while((result = patternHeadline.exec(chapterContent)) !== null) {
                    if (result[1].length === 1) {
                        var href = 'ch' + (i+1) + '_' + j + '.html';
                        chapterLinks.push('<li><a href="' + href + '">' + (i + 1) + result[2] + '</a></li>');
                    }
                }
            }
        }
    ...
    while((result = patternHeadline.exec(chapterContent)) !== null) {
        ...
        if (level < 3) {
            ...
            chapterToc = chapterToc +
                ...
                '<a href="html/' + translate.name + '/ch' + index + "_" + i + '.html">' +
                section + "  " + headline +
                '</a>' +
                '</li>';
            i++;
...
function generateChapterPages(chapterContent, index, chapters, translate) {
   ...
   // 获取title
    ...
    data.sidebar = '<ul class="nav nav-tabs nav-stacked sub-nav affix">' + '<li class="nav-header">' + h1 + '</li>' + subNavigation + '</ul>';
