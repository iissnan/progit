...
function generateOutputDir() {
    if (!fs.existsSync(pathOutputZh)) {
        fs.mkdirSync(pathOutput);
        if (!fs.existsSync(pathOutputEn)) {
            fs.mkdirSync(pathOutputEn);
        }
        if (!fs.existsSync(pathOutputZh)) {
            fs.mkdirSync(pathOutputZh);
        }
    }
}
...
// 生成关于页面
function generatePageAbout() {
    var pathPage = __dirname + "/../about.html",
        pathPageTemplate = __dirname + "/../template/about.jade",
        pathPageTemplateCompiled = jade.compile(
            fs.readFileSync(pathPageTemplate, "utf-8"),
            {filename : pathTemplateLayout}
        );

    fs.writeFileSync(pathPage, pathPageTemplateCompiled({
        title: "关于",
...
function generateBuildInfo(){
    ...
    var buildDate = current.getFullYear() + "-" +
        (current.getMonth() + 1) + "-" +
        current.getDate() + " " +
        current.getHours() + ":" +
        current.getMinutes() + ":" +
    ...
    footer = "构建时间： " + buildDate;

    var execSync = require("execSync");
    var subModuleCommit = "git submodule status";
    var buildCommit = "";

    if (!execSync.code(subModuleCommit)) {
        buildCommit = execSync.stdout(subModuleCommit);
        buildCommit = buildCommit.split(" ");
        if (buildCommit.length > 2) {
            buildCommit = buildCommit[1];
            var commitLink = '<a href="https://github.com/progit/progit/tree/' +
                buildCommit + '" target="_blank">' +
                buildCommit.substring(0, 7) + '</a>';
            footer = footer + "， 书籍源码版本： " + commitLink;
        }
    }
}
