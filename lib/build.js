...
function generateHTML(){
    ...
    fs.writeFileSync(pathIndexZh, templateIndex({
        title: "Pro Git 简体中文版",
        content: toc,
            ...
            if (chapterContent) {
                ...
                while((result = patternHeadline.exec(chapterContent)) !== null) {
                    if (result[1].length === 1) {
                        var href = (isIndex ? 'html/zh/' : '') + 'ch' + (i+1) + '_' + j + '.html';
    ...
    while((result = patternHeadline.exec(chapterContent)) !== null) {
        ...
        if (level < 3) {
            ...
            chapterToc = chapterToc +
                ...
                '<a href="html/zh/ch' + index + "_" + i + '.html">' +
                section + "  " + headline +
                '</a>' +
                '</li>';
            i++;
...
function generateChapterPages(chapterContent, index) {
   ...
   // 获取title
    ...
    for (var j = 0; j < sections.length; j++) {
        pathToPage = pathOutputZh + "/" + pagePrefix + j + ".html";
